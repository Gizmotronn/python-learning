...
* This script measures how far away the agent is from the center of the track
* A higher reward is given based on the closeness of the car to the center line
...
An agent trained with this function is likely to learn to follow the track well as it is more specific about the kind of driving behavior to reward; however behaviors like braking (for corners) or accelerating are unlikely to be learnt.
...
def reward_function(params):
    ...
    # Read input parameters
    track_width = params['track_width'] # this is set as a variable inside the DeepRacer Program
    distance_from_center = params['distance_from_center']
    ...
    return float(reward) # This depends on the distance from the center of the track
```
